...
class TestRun_cos_clusters_worker(TestCase):
    def setUp(self):
        self.run_parameters = tstdata.get_test_paramters_dictionary()
        self.run_parameters["run_directory"] = '.'
        self.run_parameters["tmp_directory"] = kn.create_dir(self.run_parameters["run_directory"], 'tmp_cc_net_cos')
    ...
    def test_run_cos_clusters_worker(self):
        ...
        for sample in range(0, int(self.run_parameters['number_of_bootstraps'])):
            sctbx.run_cc_cos_clusters_worker(spreadsheet_mat, self.run_parameters, sample)
        ...
        self.assertEqual(cluster_difference, 0, msg='cos clustering failed')
